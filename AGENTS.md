...
- 扩展身份的表达按宿主生命周期统一：Registry 会主动构造并持有全部扩展实例时，Stable ID 应属于基类/接口的实例契约；Registry 必须在构造前按 ID 路由到具体类型时，使用携带 Stable ID 的参数化 Attribute。不得仅为语法统一而实例化全部延迟扩展，也不得在同一协议中同时要求 Attribute ID 与实例 ID。
