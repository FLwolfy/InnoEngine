...
public sealed class MissingGameSystem : GameSystem
{
    ...
    internal void RestoreState(SceneElementState state)
    {
        byte[] properties = state.properties.ToArray();
        AssetDependency[] dependencies = state.GetDependencies();
        IReadOnlyDictionary<Guid, Guid> aliases = state.GetAliases();
        m_serializedState = properties;
