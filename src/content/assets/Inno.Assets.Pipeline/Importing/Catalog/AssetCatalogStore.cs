...
internal sealed class AssetCatalogStore
{
    ...
    internal void PromoteTo(string destinationLibraryRoot)
    {
        ...
        string? sourcePath = LatestPathOrNull();
        byte[]? candidate = sourcePath is null ? null : IOFile.ReadAllBytes(sourcePath);
        byte[]? previous = IOFile.Exists(destinationSnapshot) ? IOFile.ReadAllBytes(destinationSnapshot) : null;
        Directory.CreateDirectory(destinationRoot);
        ...
        try
        {
            if (candidate is null)
                ...
                AtomicFile.WriteAllBytes(destinationSnapshot, candidate);
            ...
            if (snapshotChanged)
            {
                try
                {
                    if (previous is null)
                        ...
                        AtomicFile.WriteAllBytes(destinationSnapshot, previous);
                }
                catch (Exception rollback)
                {
                    throw new AggregateException("Asset catalog promotion and snapshot compensation failed.", failure, rollback);
                }
            }
            throw;
        }
