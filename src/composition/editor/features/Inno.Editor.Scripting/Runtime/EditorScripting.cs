...
internal sealed class EditorScripting : EditorModule, IEditorScriptCompilation
{
    ...
    private PendingCompletion? m_pendingCompletion;
    ...
    internal bool isCompiling
        => m_manager?.isFaulted != true &&
           (m_showCompilation ||
            m_compilation is not null ||
            m_pendingCompletion is not null ||
    ...
    /// </summary>
    /// <returns>
    /// A ticket for this request that succeeds only after activation and retired-generation verification.
    /// </returns>
    public IScriptCompilationTicket RequestCompilation()
    {
        ...
        ScriptReloadHost? manager = m_manager;
        if (manager is null || manager.isFaulted)
        {
            ticket.MarkFailed(result: null, manager?.isFaulted == true
                ? "Generation retirement failed. Restart the host before requesting compilation."
                : "Project scripting is not available.");
            return ticket;
        }
    ...
    protected override void OnUpdate(EditorContext context)
    {
        ...
        if (m_manager is null || m_compilation is not null)
            return;
        if (m_pendingCompletion is not null || m_manager.isUnloadVerificationPending)
        {
            AdvanceUnloadVerification();
            return;
        }
    ...
    private void CompleteCompilation()
    {
        ...
        try
        {
            ...
            if (result.success)
            {
                ...
                bool activated = m_manager.ApplyPendingReload();
                if (!activated && m_manager.isCompilationPending)
                {
                    ticket?.MarkSuperseded();
                    return;
                }
                m_activationFailure = null;
                ScriptDiagnosticPublisher.ClearReload();
                CompleteAfterRetirement(new PendingCompletion(ticket, result, true,
                    "Script generation compiled, activated, and retired successfully."));
            }
            else
            {
                ...
                if (activatedUnavailableGeneration)
                {
                    ...
                    m_activationFailure = null;
                    ScriptDiagnosticPublisher.ClearReload();
                    CompleteAfterRetirement(new PendingCompletion(ticket, result, false,
        ...
        finally
        {
            ...
            m_hideCompilationOnNextUpdate = m_pendingCompletion is null &&
                m_manager?.isUnloadVerificationPending != true &&
                m_manager?.isCompilationPending != true;
        }
    }

    private void CompleteAfterRetirement(PendingCompletion completion)
    {
        if (m_manager?.isUnloadVerificationPending == true)
        {
            m_pendingCompletion = completion;
            return;
        }
    ...
    private void FinishCompletion(PendingCompletion completion)
    {
        ...
        if (completion.succeeded)
        {
            completion.ticket?.MarkSucceeded(completion.result, projected
                ? completion.status
                : "Script generation retired successfully, but IDE project generation failed.");
        }
        else
        {
            completion.ticket?.MarkFailed(completion.result, completion.status);
        }
    }
    ...
    private void AdvanceUnloadVerification()
    {
        ...
        if (manager is null || !manager.AdvanceUnloadVerification(out Exception? failure))
            return;
        PendingCompletion? completion = m_pendingCompletion;
        m_pendingCompletion = null;
        if (failure is not null)
        {
            string status = $"Generation retirement failed. Restart the host before Play, Build, Export or reload: {failure.Message}";
            completion?.ticket?.MarkFailed(completion.result, status);
            m_currentTicket?.MarkFailed(completion?.result, status);
            if (m_activationFailure != status)
            {
                ScriptDiagnosticPublisher.PublishUnloadFailure(failure);
            }
            m_activationFailure = status;
            m_showCompilation = false;
            m_hideCompilationOnNextUpdate = false;
            return;
        }
