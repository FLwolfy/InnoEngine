...
internal sealed class ShaderGraphPanel : EditorPanel
{
    ...
    private bool EnsureDocument()
    {
        ...
        if (found && asset?.document is not null)
        {
        ...
        if (m_documentId == Guid.Empty) m_documentId = Guid.NewGuid();
        ...
        if (!m_graphs.TryOpenDocument(m_documentId, m_interactions.history, out m_controller))
            m_controller = m_graphs.OpenDocument(m_documentId, asset?.document?.Clone() ?? new GraphDocument(), m_interactions.history);
        return true;
    }
    ...
    private void DrawCompileStatus()
    {
        ...
        ShaderDiagnostic? diagnostic = m_compileResult.diagnostics.FirstOrDefault(
            static value => value.severity == DiagnosticSeverity.Error);
    ...
    private void Save(EditorContext context)
    {
        ...
        if (m_sourceAssigned)
        {
            if (!m_assets.TryLoad(m_documentId, out ShaderGraphAsset? current) || current is null)
                throw new InvalidOperationException("Cannot save a Missing Shader Graph source.");
            asset = current;
        }
        else
        {
            asset = new ShaderGraphAsset();
            m_assets.identities.InitializePersistentIdentity(asset, m_documentId);
        }
        GraphDocument? previous = asset.document?.Clone();
        asset.SetDocument(m_controller!.document, m_serialization);
        try
        {
        ...
        {
            if (previous is not null) asset.SetDocument(previous, m_serialization);
            throw;
        }
        m_sourceAssigned = true;
