...
public sealed class PluginSourceService
{
    ...
    /// <summary>Creates a local Plugin source service.</summary>
    /// <param name="pluginRoot">Project Plugins directory containing ZIPs and unpacked Plugin directories.</param>
    /// <param name="libraryRoot">Project rebuildable Library directory.</param>
    /// <param name="limits">Optional bounded source limits.</param>
    ...
    /// <summary>Scans every installed ZIP and directory into one isolated candidate snapshot.</summary>
    ...
    public PluginScanResult Scan()
    {
        ...
        foreach (PluginSource source in DiscoverSources())
        {
            try
            {
                candidates.Add(source.kind switch
                {
    ...
    /// <summary>Gets read-only mounts that passed validation and dependency ordering.</summary>
    ...
    /// <summary>Gets validated candidates in complete dependency order.</summary>
    /// <param name="result">Complete scan result.</param>
    ...
    private PluginCandidate ValidateZip(string zipPath)
    {
        ...
        manifest.Validate();
        ValidateContent(entries.Select(static entry => entry.path).ToArray(), "ZIP");
    ...
    private PluginCandidate ValidateDirectory(string directoryPath)
    {
        ...
        ValidatedDirectoryEntry[] entries = ValidateDirectoryEntries(directoryPath).ToArray();
        ValidatedDirectoryEntry manifestEntry = entries.SingleOrDefault(static value => value.path == "Plugin.inno")
            ?? throw new InvalidDataException("A Plugin directory requires exactly one root Plugin.inno manifest.");
        PluginManifest manifest = ReadManifest(manifestEntry.physicalPath);
        manifest.Validate();
        ValidateContent(entries.Select(static entry => entry.path).ToArray(), "directory");

        string contentHash = ComputeDirectoryContentHash(entries);
        string contentRoot = Path.Combine(directoryPath, "Assets");
        bool containsCode = ContainsCode(entries.Select(static entry => entry.path));
        return CreateCandidate(
            directoryPath,
    ...
    private static void ValidateContent(IReadOnlyList<string> entries, string sourceDescription)
    {
    ...
    private static PluginManifest ReadManifest(ZipArchiveEntry entry)
    {
        try
        {
            ...
            return SerializationManager.Deserialize<PluginManifest>(memory.ToArray());
        }
    ...
    private static PluginManifest ReadManifest(string path)
    {
        try
        {
            return SerializationManager.Deserialize<PluginManifest>(IOFile.ReadAllBytes(path));
        }
        catch (Exception exception) when (IsManifestDataFailure(exception))
        {
            throw new InvalidDataException("Plugin.inno is malformed or incompatible with the current contract.", exception);
