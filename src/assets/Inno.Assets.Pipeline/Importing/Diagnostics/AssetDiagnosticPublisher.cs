...
internal sealed class AssetDiagnosticPublisher : IDisposable
{
    ...
    internal void SynchronizeImports(IReadOnlyList<AssetMeta> entries)
    {
        ...
        {
            m_diagnostics.Clear(CreateSource(C_IMPORT_GROUP, resolved[i]));
        ...
        IReadOnlyList<string> messages)
    {
        ...
        m_diagnostics.Set(CreateSource(C_BUILD_GROUP, targetId, displayName), diagnostics);
    ...
    internal void ResolveCatalog()
    {
        if (string.IsNullOrEmpty(m_catalogState))
            return;
        m_diagnostics.Clear(CreateSource(C_CATALOG_GROUP));
    ...
    public void Dispose()
    {
        ...
        foreach (Guid targetId in m_importStates.Keys)
            m_diagnostics.Clear(CreateSource(C_IMPORT_GROUP, targetId));
        foreach (Guid targetId in m_buildStates.Keys)
            m_diagnostics.Clear(CreateSource(C_BUILD_GROUP, targetId));
        foreach (Guid targetId in m_referenceStates.Keys)
            m_diagnostics.Clear(CreateSource(C_REFERENCE_GROUP, targetId));
