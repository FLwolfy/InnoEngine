...
internal sealed class AssetArtifactStore
{
    ...
    internal AssetArtifactStore(string libraryRoot)
    {
        ...
        out AssetArtifactInfo? artifact)
    {
        ...
        if (key.isEmpty || string.IsNullOrWhiteSpace(outputName))
            return false;
        AssetArtifactManifest? manifest = ReadManifest(key);
    ...
    private AssetArtifactManifest? ReadManifest(AssetArtifactKey key)
    {
        ...
        try
        {
            return SerializationManager.Deserialize<AssetArtifactManifest>(IOFile.ReadAllBytes(path));
        }
        catch
        {
            return null;
