...
namespace Inno.Editor.Panel.Inspector.Interactions;

[EditorDrop(InspectorAreas.EngineObjectReference, priority: 100)]
    ...
    : EditorDrop<EngineObject, EngineObjectReferenceDropTarget>
{
        ...
        EditorDropContext<EngineObject, EngineObjectReferenceDropTarget> context)
    {
        ...
        if (source is GameObject sourceObject &&
            (!sourceObject.isRuntimeValid || !ReferenceEquals(sourceObject.scene, SceneManager.activeScene)))
        {
            return null;
        }
