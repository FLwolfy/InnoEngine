...
internal static class ScriptApiStubSourceBuilder
{
        ...
        IReadOnlySet<Type> exportedTypes)
    {
        ...
        try
        {
            ...
            Version version = export.assembly.GetName().Version ?? new Version(1, 0, 0, 0);
            builder.AppendLine("// <auto-generated/>");
            builder.AppendLine("#nullable enable");
            builder.Append("[assembly: global::System.Reflection.AssemblyVersion(\"")
                .Append(version)
                .AppendLine("\")]");
            foreach (Type type in export.exportedTypes.OrderBy(static value => value.FullName, StringComparer.Ordinal))
            {
    ...
    private static void AppendProperties(StringBuilder builder, Type type)
    {
        foreach (PropertyInfo property in type.GetProperties(C_DECLARED_MEMBERS))
        {
            ...
            builder.AppendLine().AppendLine("    {");
            AppendAccessor(builder, "get", getter, accessibility, type.IsInterface);
            AppendAccessor(builder, "set", setter, accessibility, type.IsInterface);
            builder.AppendLine("    }");
        }
    }
        ...
        bool declaringTypeIsInterface)
    {
        ...
        if (declaringTypeIsInterface || method.IsAbstract)
            builder.AppendLine(";");
        else
            builder.AppendLine(" => throw new global::System.NotImplementedException();");
    }
    ...
    private static void AppendMethods(StringBuilder builder, Type type)
    {
        foreach (MethodInfo method in type.GetMethods(C_DECLARED_MEMBERS))
        {
            ...
            AppendMethodBody(builder, method, type.IsInterface);
        }
    }
    ...
    private static void AppendMethodBody(StringBuilder builder, MethodInfo method, bool declaringTypeIsInterface)
    {
        if (declaringTypeIsInterface || method.IsAbstract)
            builder.AppendLine(";");
        else
    ...
    private static string FormatType(Type type)
    {
        ...
        string name = RemoveGenericArities(definition.FullName ?? definition.Name).Replace('+', '.');
        return "global::" + name + "<" +
