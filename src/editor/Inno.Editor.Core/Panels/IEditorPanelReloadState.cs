...
public interface IEditorPanelReloadState
{
    ...
    /// </summary>
    /// <returns>An immutable byte payload that can be consumed by the replacement panel generation.</returns>
    ReadOnlyMemory<byte> CaptureReloadState();

    /// <summary>
    /// Restores presentation-neutral state captured from the previous panel generation.
    /// </summary>
    /// <param name="state">The immutable payload returned by the retiring panel generation.</param>
