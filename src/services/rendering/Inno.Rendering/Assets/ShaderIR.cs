...
public sealed class ShaderIRStageModule
{
    ...
    /// </summary>
    /// <param name="stage">
    /// Single shader stage represented by the source.
    /// </param>
    /// <param name="entryPoint">
    /// Entry point passed to the target compiler.
    /// </param>
    /// <param name="source">
    ...
    /// <summary>
    /// Gets the single shader stage.
    /// </summary>
    public ShaderStage stage { get; }

    /// <summary>
    /// Gets the compiler entry point.
    /// </summary>
    public string entryPoint { get; }

    /// <summary>
    /// Gets shaderc-compatible canonical source.
    /// </summary>
...
public sealed class ShaderIRPass
{
    ...
    /// Pass interface and fixed-function state.
    /// </param>
    /// <param name="stages">
    ...
    /// <summary>
    /// Gets the pass interface and fixed-function state with detached metadata storage.
    /// </summary>
    public ShaderPassDefinition definition => ShaderDefinitionSnapshot.Copy(m_definition);

    /// <summary>
    /// Gets canonical stage modules.
    /// </summary>
...
{
    private readonly ShaderDefinitionSnapshot m_definition;

    /// <summary>
    /// Creates a shader IR module.
    /// </summary>
    /// <param name="definition">
    /// Stable properties, keywords and pass manifest.
    /// </param>
    /// <param name="passes">
    ...
    public ShaderIRModule(ShaderDefinition definition, IReadOnlyList<ShaderIRPass> passes)
    {
        ...
        m_definition = new ShaderDefinitionSnapshot(definition);
        this.passes = Array.AsReadOnly(passes.ToArray());
    }

    /// <summary>
    /// Gets detached stable properties, keywords and pass declarations for this compiled input.
    /// </summary>
    public ShaderDefinition definition => m_definition.CreateDefinition();

    /// <summary>
    /// Gets canonical pass stage implementations.
    /// </summary>
...
public sealed class ShaderInterface
{
    ...
    /// <summary>
    /// Builds an interface from a validated shader manifest.
    /// </summary>
    /// <param name="module">
    /// Validated shader IR.
    /// </param>
    /// <returns>
    /// The expected runtime binding interface.
    /// </returns>
    ...
    /// <summary>
    /// Builds the interface consumed by one validated shader pass.
    /// </summary>
    /// <param name="module">
    /// Validated shader IR containing the pass and property manifest.
    /// </param>
    /// <param name="pass">
    /// Pass whose stage-local interface is required.
    /// </param>
    /// <returns>
    /// The ordered binding interface used to create and reflect this pass.
    /// </returns>
...
public static class ShaderIRValidator
{
    ...
    /// </summary>
    /// <param name="module">
    /// Candidate shader IR module.
    /// </param>
    /// <param name="capabilities">
    /// Optional target capability snapshot.
    /// </param>
    /// <returns>
    /// Structured diagnostics suitable for source and graph navigation.
    /// </returns>
        ...
        List<ShaderDiagnostic> diagnostics)
    {
        ...
        if (capabilities is not null
            && (definition.requiredFeatures & ~capabilities.features) != GraphicsCapability.None)
        {
            diagnostics.Add(Error(
