...
public enum ShaderValueType
{
    ...
    Matrix4x4,
    /// <summary>
    /// Two-dimensional sampled texture.
    /// </summary>
    Texture2D,
    /// <summary>
    /// Layered two-dimensional sampled texture.
    /// </summary>
    Texture2DArray,
    /// <summary>
    /// Three-dimensional sampled volume texture.
    /// </summary>
    Texture3D,
    /// <summary>
    /// Cube sampled texture.
    /// </summary>
    TextureCube,
    /// <summary>
    /// Sampler state.
...
{
    /// <summary>
    /// Gets the stable type ID for one shader value type.
    /// </summary>
    /// <param name="type">
    /// Shader value type.
    /// </param>
    /// <returns>
    /// A stable graph type identifier.
    /// </returns>
    public static string GetId(ShaderValueType type) => $"inno.shader.{type}";

    /// <summary>
    /// Parses a stable shader graph type identifier.
    /// </summary>
    /// <param name="typeId">
    /// Stable graph type identifier.
    /// </param>
    /// <returns>
    /// The represented shader value type.
    /// </returns>
    /// <exception cref="ArgumentException">
    /// Thrown when the identifier is not a shader value type.
    /// </exception>
...
{
    /// <summary>
    /// Determines whether a registered conversion connects the two stable shader value types.
    /// </summary>
    /// <param name="sourceTypeId">
    /// The source type id text validated by the can convert operation.
    /// </param>
    /// <param name="destinationTypeId">
    /// The destination type id text validated by the can convert operation.
    /// </param>
    /// <returns>
...
{
    /// <summary>
    /// Creates a shader node extension declaration.
    /// </summary>
    /// <param name="id">
    /// Globally stable node definition identifier.
    /// </param>
    ...
    /// <summary>
    /// Gets the globally stable node definition identifier.
    /// </summary>
...
{
    /// <summary>
    /// Creates a typed shader expression.
    /// </summary>
    /// <param name="type">
    /// Static expression type.
    /// </param>
    /// <param name="expression">
    /// Shaderc-compatible expression text.
    /// </param>
    ...
    /// <summary>
    /// Gets the static expression type.
    /// </summary>
    public ShaderValueType type { get; }

    /// <summary>
    /// Gets shaderc-compatible expression text.
    /// </summary>
    public string expression { get; }

    /// <summary>
    /// Gets the stable source node identity.
    /// </summary>
...
{
    /// <summary>
    /// Creates a shader node definition.
    /// </summary>
    /// <param name="id">
    /// Globally stable definition identity.
    /// </param>
    ...
    /// </param>
    /// <param name="supportedStages">
    /// Shader stages in which the node may emit code.
    /// </param>
    ...
    /// <summary>
    /// Gets shader stages in which the node may emit code.
    /// </summary>
    public ShaderStage supportedStages { get; }

    /// <summary>
    /// Emits typed expressions, statements, properties or output semantics.
    /// </summary>
    /// <param name="context">
    /// Generation-scoped node emission context.
    /// </param>
...
public sealed class ShaderNodeEmitContext
{
    ...
    /// <summary>
    /// Gets the neutral source node.
    /// </summary>
    public GraphNodeRecord node { get; }

    /// <summary>
    /// Gets the stage currently being emitted.
    /// </summary>
    public ShaderStage stage { get; }

    /// <summary>
    /// Reads one optional node property through the serialization generation captured for this compilation.
    /// </summary>
    /// <typeparam name="T">
    /// The declared property value type.
    /// </typeparam>
    /// <param name="key">
    /// The stable property key stored by the node record.
    /// </param>
    /// <param name="defaultValue">
    /// The value returned when the node does not contain the requested property.
    /// </param>
    /// <returns>
    /// The deserialized property value, or <paramref name="defaultValue"/> when the property is absent.
    /// </returns>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="key"/> is empty or contains only whitespace.
    /// </exception>
    public T ReadValue<T>(string key, T defaultValue)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(key);
        return node.TryGetValue(key, out GraphSerializedValue? serialized)
            ? serialized!.Deserialize<T>(m_serialization)
            : defaultValue;
    }

    /// <summary>
    /// Gets a required connected input.
    /// </summary>
    /// <param name="portId">
    /// Stable input port identity.
    /// </param>
    /// <returns>
    /// The converted upstream shader value.
    /// </returns>
    /// <exception cref="InvalidOperationException">
    /// Thrown when no input value is connected.
    /// </exception>
    ...
    /// <summary>
    /// Tries to get a connected input.
    /// </summary>
    /// <param name="portId">
    /// Stable input port identity.
    /// </param>
    /// <param name="value">
    /// Receives the converted upstream shader value.
    /// </param>
    /// <returns>
    /// <see langword="true"/> when the input is connected.
    /// </returns>
    public bool TryGetInput(GraphPortId portId, out ShaderValue value)
        => m_inputs.TryGetValue(portId, out value);

    /// <summary>
    /// Publishes one typed node output expression.
    /// </summary>
    /// <param name="portId">
    /// Stable output port identity.
    /// </param>
    /// <param name="value">
    ...
    /// <summary>
    /// Adds one complete statement to the current stage body.
    /// </summary>
    /// <param name="statement">
    /// Shaderc-compatible statement.
    /// </param>
    ...
    /// <summary>
    /// Declares one stable material property in the shared Shader IR manifest.
    /// </summary>
    /// <param name="property">
    ...
    /// <summary>
    /// Publishes one output semantic consumed by the graph target generator.
    /// </summary>
    /// <param name="semantic">
    /// Stable target semantic.
    /// </param>
    /// <param name="value">
...
/// <summary>
/// Contains one stage-local graph emission requested by a Plugin program output.
/// </summary>
public sealed class ShaderGraphEmission
{
        ...
        GraphNodeId outputNodeId)
    {
        ...
        this.semantics = new System.Collections.ObjectModel.ReadOnlyDictionary<string, ShaderValue>(
            new Dictionary<string, ShaderValue>(semantics, StringComparer.Ordinal));
        this.statements = Array.AsReadOnly(statements.ToArray());
        this.outputNodeId = outputNodeId;
    }

    /// <summary>
    /// Gets the concrete stage used for this emission.
    /// </summary>
    public ShaderStage stage { get; }

    /// <summary>
    /// Gets stable properties declared by participating nodes.
    /// </summary>
    public IReadOnlyList<ShaderPropertyDefinition> properties { get; }

    /// <summary>
    /// Gets open output semantics published by the program node.
    /// </summary>
    public IReadOnlyDictionary<string, ShaderValue> semantics { get; }

    /// <summary>
    /// Gets ordered shaderc-compatible statements emitted by participating nodes.
    /// </summary>
    public IReadOnlyList<string> statements { get; }

    /// <summary>
    /// Gets the stable program output node identity.
    /// </summary>
    public GraphNodeId outputNodeId { get; }

    /// <summary>
    /// Gets a required open output semantic.
    /// </summary>
    /// <param name="id">
    /// Plugin-defined semantic ID.
    /// </param>
    /// <returns>
    /// The typed emitted value.
    /// </returns>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the output did not publish the semantic.
    /// </exception>
...
/// <summary>
/// Exposes validated graph emission to one Plugin-defined program output node.
/// </summary>
public sealed class ShaderGraphProgramContext
{
    ...
    /// <summary>
    /// Gets the canonical graph asset path used by diagnostics.
    /// </summary>
    public string assetPath { get; }

    /// <summary>
    /// Gets the artist-facing generated shader name.
    /// </summary>
    public string shaderName { get; }

    /// <summary>
    /// Gets the compile-scoped authoring graph borrowed for Plugin-specific metadata inspection.
    /// </summary>
    public GraphDocument document { get; }

    /// <summary>
    /// Gets the selected Plugin program output node.
    /// </summary>
    public GraphNodeRecord outputNode { get; }

    /// <summary>
    /// Emits all ancestors of the program output for one concrete shader stage.
    /// </summary>
    /// <returns>
    /// Typed properties, semantics, and statements for that stage.
    /// </returns>
    /// <param name="stage">
    /// The stage consumed by emit; ownership remains with the caller unless explicitly stated otherwise.
    /// </param>
    public ShaderGraphEmission Emit(ShaderStage stage) => m_emit(stage);

    /// <summary>
    /// Creates a generated stage module with stable output-node source mapping.
    /// </summary>
    /// <param name="passName">
    /// Owning pass name.
    /// </param>
    /// <param name="stage">
    /// Concrete shader stage.
    /// </param>
    /// <param name="source">
    /// Complete shaderc-compatible stage source.
    /// </param>
    /// <param name="emission">
    /// Emission used to generate the source.
    /// </param>
    /// <param name="entryPoint">
    /// Compiler entry point.
    /// </param>
    /// <returns>
    /// A shared Shader IR stage module.
    /// </returns>
...
/// <summary>
/// Defines the sole graph output that turns open node semantics into arbitrary shared Shader IR.
/// </summary>
public abstract class ShaderGraphProgramNodeDefinition : ShaderNodeDefinition
{
    /// <summary>
    /// Creates a Plugin-defined program output.
    /// </summary>
    /// <param name="id">
    /// Globally stable node identity.
    /// </param>
    /// <param name="displayName">
    /// Artist-facing display name.
    /// </param>
    ...
    /// </param>
    /// <param name="supportedStages">
    /// Stages that the output may request from its ancestors.
    /// </param>
    ...
    /// <summary>
    /// Builds arbitrary passes, techniques, contracts, and stage sources from graph emissions.
    /// </summary>
    /// <param name="context">
    /// Generation-scoped program build context.
    /// </param>
    /// <returns>
    /// The complete shared Shader IR module.
    /// </returns>
...
public sealed class ShaderNodeRegistry : IGraphNodeDefinitionResolver, IDisposable
{
    ...
    private ShaderNodeExtensionRegistry.Snapshot? m_retired;
    private RetirementBarrier? m_replacementBarrier;
    private LifetimeScope? m_retirement;
    private RetirementBarrier? m_shutdown;

    /// <summary>
    /// Creates a manually populated shader node registry.
    /// </summary>
    ...
    /// <summary>
    /// Creates a registry transactionally populated from one type catalog.
    /// </summary>
    /// <param name="types">
    /// The type catalog that owns discovered Shader Node generations.
    /// </param>
    public ShaderNodeRegistry(Inno.Extensibility.Types.TypeCatalog types)
        : this()
    {
        ArgumentNullException.ThrowIfNull(types);
        m_extensions = new ShaderNodeExtensionRegistry(this, types);
    }

    /// <summary>
    /// Gets active definitions in stable identity order.
    /// </summary>
    ...
    /// <summary>
    /// Gets the monotonic active definition generation used to invalidate graph previews.
    /// </summary>
    ...
    /// <summary>
    /// Builds and atomically activates a complete candidate definition snapshot.
    /// </summary>
    /// <param name="definitions">
    /// Generation-scoped Plugin definitions.
    /// </param>
    /// <exception cref="ArgumentException">
    /// Thrown when the candidate contains duplicate IDs.
    /// </exception>
    public void Replace(IEnumerable<ShaderNodeDefinition> definitions)
    {
        ...
        DrainRetired();
        ShaderNodeDefinition[] snapshot = definitions.ToArray();
        if (snapshot.Any(definition => m_active.definitions.Values.Any(active => ReferenceEquals(active, definition))))
            throw new ArgumentException("Replacement definitions must have independent generation ownership.", nameof(definitions));
        ShaderNodeExtensionRegistry.Snapshot candidate = ShaderNodeExtensionRegistry.Snapshot.Create(snapshot);
        m_retired = Activate(candidate);
        DrainRetired();
    }

    /// <summary>
    /// Builds the initial extension generation from the active TypeCache snapshot.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Attempts to resolve without changing state when the operation cannot complete.
    /// </summary>
    /// <param name="definitionId">
    /// The definition id text validated by the try resolve operation.
    /// </param>
    /// <param name="definition">
    /// The definition consumed by try resolve; ownership remains with the caller unless explicitly stated otherwise.
    /// </param>
    /// <returns>
    /// <see langword="true"/> when the operation succeeds or its condition is satisfied; otherwise, <see langword="false"/>.
    /// </returns>
    ...
    /// <summary>
    /// Tries to resolve one active shader node definition.
    /// </summary>
    /// <param name="definitionId">
    /// Stable definition identity.
    /// </param>
    /// <param name="definition">
    /// Receives the active shader definition.
    /// </param>
    /// <returns>
    /// <see langword="true"/> when the definition is active.
    /// </returns>
    ...
    /// <summary>
    /// Releases the active node generation and unregisters extension discovery.
    /// </summary>
    public void Dispose()
    {
        ...
        if (m_retirement is null)
        {
            ...
            if (m_extensions?.isInitialized != true) m_retirement.Own(m_active);
            if (m_extensions is not null) m_retirement.Own(m_extensions);
            if (m_retired is not null) m_retirement.Own(m_retired);
            m_shutdown = new RetirementBarrier("Shader node registry shutdown");
        }
        try { m_shutdown!.Wait(m_retirement.Dispose); }
        catch (Exception pending) when (RetirementPendingException.Find(pending) is not null) { throw; }
        catch { FinishRetirement(); throw; }
        FinishRetirement();
    }
    ...
    {
        if (m_retired is null) return;
        m_replacementBarrier ??= new RetirementBarrier("Shader node definition replacement");
        try { m_replacementBarrier.Wait(m_retired.Dispose); }
        catch (Exception pending) when (RetirementPendingException.Find(pending) is not null) { throw; }
        catch { m_retired = null; m_replacementBarrier = null; throw; }
        m_retired = null;
        m_replacementBarrier = null;
    }
