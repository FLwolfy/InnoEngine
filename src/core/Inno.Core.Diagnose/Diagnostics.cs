...
public static class Diagnostics
{
    ...
    /// </summary>
    /// <param name="group">The non-empty responsibility name scoped to the calling type.</param>
    /// <param name="diagnostic">The single current diagnostic. It replaces every previous entry in the group.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="group"/> is empty.</exception>
    ...
    /// </param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="group"/> is empty or the collection contains a null entry.</exception>
    ...
    /// </summary>
    /// <param name="targetId">The stable identity of the asset, scene element, or other dynamic target.</param>
    /// <param name="group">The non-empty responsibility name scoped to the calling type and target.</param>
    /// <param name="diagnostic">The single current diagnostic. It replaces every previous entry in the targeted group.</param>
    /// <param name="displayName">The optional target name presented by diagnostic consumers.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="targetId"/> is empty or <paramref name="group"/> is empty.</exception>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="diagnostic"/> is <see langword="null"/>.</exception>
    [MethodImpl(MethodImplOptions.NoInlining)]
    ...
    public static void Clear(string group)
        => DiagnosticManager.Clear(DiagnosticCallerResolver.Resolve(group));
    ...
    public static void Clear(Guid targetId, string group)
        => DiagnosticManager.Clear(DiagnosticCallerResolver.Resolve(group, targetId));
}
