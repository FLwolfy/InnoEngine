...
public sealed class ScriptReloadHost : IDisposable
{
    ...
    private bool m_publishingGeneration;
        ...
        IScriptReloadCoordinator reloads)
    {
        ...
        if (options.unloadCollectionInterval < TimeSpan.Zero)
            throw new ArgumentOutOfRangeException(nameof(options), "Unload collection interval cannot be negative.");
        if (options.unloadRetentionTimeout <= TimeSpan.Zero &&
            options.unloadRetentionTimeout != Timeout.InfiniteTimeSpan)
        {
            throw new ArgumentOutOfRangeException(
                nameof(options),
                "Unload retention timeout must be positive or infinite.");
        }
        m_assets = assets;
        m_compiler = compiler;
        m_plugins = plugins;
        ...
        m_options = new ScriptReloadOptions
        {
            ...
            unloadCollectionInterval = options.unloadCollectionInterval,
            unloadRetentionTimeout = options.unloadRetentionTimeout
        };
        m_modules.generations.Configure(new AssemblyUnloadBarrierOptions
        {
            collectionInterval = options.unloadCollectionInterval,
            retentionTimeout = options.unloadRetentionTimeout
        });
    ...
    public bool isUnloadVerificationPending
    {
        ...
        {
            lock (m_sync)
                return m_modules.generations.state == GenerationState.AwaitingCollection;
        }
    }

    /// <summary>
    /// Gets whether a retired generation exceeded the retention threshold and permanently stopped this host.
    /// </summary>
    public bool isFaulted
    {
        ...
        {
            lock (m_sync)
                return m_modules.generations.state == GenerationState.Faulted;
        }
    }
    ...
    public bool TryCompilePending(out Task<ScriptCompilationResult>? compilation)
    {
        ...
        lock (m_sync)
        {
            ...
            if (m_modules.generations.state == GenerationState.AwaitingCollection)
            {
                compilation = null;
                return false;
            }
    ...
    public bool ApplyPendingReload()
    {
        ...
        lock (m_sync)
        {
            ThrowIfFaultedLocked();
            if (m_modules.generations.state == GenerationState.AwaitingCollection)
                throw new InvalidOperationException("A new script generation cannot activate before the previous unload barrier completes.");
        ...
        string completedStatus = pending.unavailability is null
            ...
            : "Plugin availability change committed. Unavailable scene types are preserved as Missing until their " +
              "Stable IDs return.";
        SetCompilationProgress(
            !isUnloadVerificationPending ? 1f : C_UNLOAD_VERIFICATION_PROGRESS,
            !isUnloadVerificationPending
                ? completedStatus
    ...
    public void Dispose()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    // Shutdown is a new module publication and must not overlap a previous GC barrier.
    ...
    {
        failure = m_modules.generations.failure;
        if (failure is not null)
        {
            SetCompilationProgress(1f, "Generation retirement failed. Restart the host before Play, Build, Export or reload.");
            return true;
        }
        try
        {
            if (!m_modules.generations.Advance())
            {
                SetCompilationProgress(C_UNLOAD_VERIFICATION_PROGRESS, "Verifying retired generation unload...");
                return false;
            }
            SetCompilationProgress(1f, "Generation publication and retired assembly unload completed.");
            return true;
        }
        catch (Exception exception)
        {
    ...
    {
        if (m_disposed || !m_options.autoCompile || m_publishingGeneration)
            return;
    ...
    private AssemblyUnloadMonitor PublishGeneration(AssemblyReloadSession reload)
    {
        ...
        m_publishingGeneration = true;
        try
        {
            return m_reloads.Execute(reload, m_plugins.CreateReloadChange());
        }
        finally
        {
            m_publishingGeneration = false;
        }
    }
    ...
    {
        if (m_modules.generations.state == GenerationState.Faulted)
            throw new InvalidOperationException("The generation owner is Faulted; restart the host.", m_modules.generations.failure);
    }
