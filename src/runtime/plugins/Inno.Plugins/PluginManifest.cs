...
{
    /// <summary>
    /// Gets or sets the globally stable lowercase Plugin ID.
    /// </summary>
    [SerializableProperty]
    public string pluginId { get; set; } = string.Empty;

    /// <summary>
    /// Gets or sets the artist-facing display name.
    /// </summary>
    [SerializableProperty]
    public string displayName { get; set; } = string.Empty;

    /// <summary>
    /// Gets or sets Plugin IDs that must activate before this Plugin.
    /// </summary>
    [SerializableProperty]
    public string[] dependencies { get; set; } = [];

    /// <summary>
    /// Gets or sets dependency Plugin IDs whose contributions may be explicitly replaced.
    /// </summary>
    [SerializableProperty]
    public string[] overrides { get; set; } = [];

    /// <summary>
    /// Gets or sets source-local content roots relative to the Plugin container.
    /// </summary>
    [SerializableProperty]
    public string[] contentRoots { get; set; } = ["Assets"];

    /// <summary>
    /// Gets or sets source-local assembly definition entries.
    /// </summary>
    [SerializableProperty]
    public string[] assemblyDefinitions { get; set; } = [];

    /// <summary>
    /// Gets or sets default project setting contributions.
    /// </summary>
    [SerializableProperty]
    public ProjectSettingRecord[] settingContributions { get; set; } = [];

    /// <summary>
    /// Validates stable identity and dependency declarations.
    /// </summary>
    /// <exception cref="InvalidDataException">
    /// Thrown when manifest data is invalid.
    /// </exception>
    public void Validate()
    {
        try
        {
            _ = new Inno.Assets.AssetSourceId(pluginId);
        }
        catch (ArgumentException exception)
        {
            throw new InvalidDataException("Plugin.inno contains an invalid Plugin ID.", exception);
        }
        if (string.Equals(pluginId, Inno.Assets.AssetSourceId.project.value, StringComparison.Ordinal))
