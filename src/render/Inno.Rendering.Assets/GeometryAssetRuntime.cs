...
public sealed class GeometryData
{
    ...
    /// <param name="sections">Contiguous submesh ranges.</param>
...
public static class GeometryAssetRuntime
{
    ...
    public static GeometryData GetGeometryData(GeometryAsset geometry)
    {
        ...
        return GeometryArtifactCodec.Decode(geometry.runtimePayload.Span);
    }
}

internal static class GeometryArtifactCodec
{
    private const ulong C_MAGIC = 0x4853454D4F4E4E49;

    internal static byte[] Encode(GeometryData data)
    {
    ...
    internal static GeometryData Decode(ReadOnlySpan<byte> bytes)
    {
        if (bytes.IsEmpty)
        {
