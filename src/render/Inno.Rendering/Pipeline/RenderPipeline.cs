...
/// <summary>Marks a reloadable render pipeline implementation with a stable extension identifier.</summary>
[AttributeUsage(AttributeTargets.Class, AllowMultiple = false, Inherited = false)]
public sealed class RenderPipelineExtensionAttribute : Attribute
{
    /// <summary>Creates a pipeline extension declaration.</summary>
...
/// <summary>Marks a reloadable pipeline feature implementation with a stable extension identifier.</summary>
[AttributeUsage(AttributeTargets.Class, AllowMultiple = false, Inherited = false)]
public sealed class RenderFeatureExtensionAttribute : Attribute
{
    /// <summary>Creates a feature extension declaration.</summary>
...
/// <summary>Indicates the impact of a rendering diagnostic.</summary>
public enum RenderDiagnosticSeverity
{
    /// <summary>Provides non-blocking information.</summary>
    Info,
    /// <summary>Reports a capability reduction or recoverable problem.</summary>
    Warning,
    /// <summary>Reports a failed request, extension, pass, or resource.</summary>
    Error
}

/// <summary>Reports one structured rendering problem without backend-native data.</summary>
public sealed class RenderDiagnostic
{
    /// <summary>Creates a rendering diagnostic.</summary>
    /// <param name="code">Stable machine-readable code.</param>
    /// <param name="message">Actionable user-facing message.</param>
    /// <param name="severity">Diagnostic impact.</param>
    /// <param name="sourceId">Optional stable asset or extension identity.</param>
    ...
    /// <summary>Gets the actionable message.</summary>
    public string message { get; }

    /// <summary>Gets diagnostic impact.</summary>
    public RenderDiagnosticSeverity severity { get; }

    /// <summary>Gets an optional stable source identity.</summary>
    public string? sourceId { get; }
}

/// <summary>Receives deduplicated rendering diagnostics.</summary>
public interface IRenderDiagnosticSink
{
    /// <summary>Publishes current diagnostic state.</summary>
    /// <param name="diagnostic">Diagnostic to publish.</param>
    void Publish(RenderDiagnostic diagnostic);
}

/// <summary>Stores open semantic graph resources for one pipeline request.</summary>
public sealed class RenderResourceRegistry
{
    ...
    /// <summary>Publishes a texture under a pipeline-defined semantic identifier.</summary>
    /// <param name="id">Open semantic resource identifier.</param>
    /// <param name="texture">Valid current-graph texture.</param>
    ...
    /// <summary>Publishes a buffer under a pipeline-defined semantic identifier.</summary>
    /// <param name="id">Open semantic resource identifier.</param>
    /// <param name="buffer">Valid current-graph buffer.</param>
    ...
    /// <summary>Tries to get a published texture.</summary>
    /// <param name="id">Open semantic resource identifier.</param>
    /// <param name="texture">Receives the current-graph texture.</param>
    /// <returns><see langword="true"/> when the texture has been published.</returns>
    public bool TryGetTexture(RenderResourceId id, out RenderTextureHandle texture)
        => m_textures.TryGetValue(id, out texture);

    /// <summary>Tries to get a published buffer.</summary>
    /// <param name="id">Open semantic resource identifier.</param>
    /// <param name="buffer">Receives the current-graph buffer.</param>
    /// <returns><see langword="true"/> when the buffer has been published.</returns>
    public bool TryGetBuffer(RenderResourceId id, out RenderBufferHandle buffer)
        => m_buffers.TryGetValue(id, out buffer);
}

/// <summary>Supplies one request and frame-scoped services to a render pipeline.</summary>
public sealed class RenderPipelineContext
{
    /// <summary>Creates a pipeline build context.</summary>
    /// <param name="request">Pipeline-defined render request.</param>
    /// <param name="pipelineAsset">Selected persistent pipeline configuration.</param>
    ...
    /// <summary>Gets selected pipeline configuration.</summary>
    ...
    /// <summary>Gets current device capabilities.</summary>
    public GraphicsCapabilities capabilities { get; }

    /// <summary>Gets open semantic resources for this request.</summary>
    ...
    /// <summary>Gets the imported offscreen target, or an invalid handle for the backbuffer.</summary>
    public RenderTextureHandle outputTexture { get; }
}

/// <summary>Supplies one configured feature with frame-scoped graph services.</summary>
public sealed class RenderFeatureContext
{
    /// <summary>Creates a feature build context.</summary>
    ...
    /// <summary>Gets stable feature configuration.</summary>
    public RenderFeatureConfiguration configuration { get; }

    /// <summary>Gets the current frame graph builder.</summary>
    public RenderGraphBuilder graph => pipeline.graph;

    /// <summary>Gets open semantic resources for the request.</summary>
...
public abstract class RenderPipeline : IDisposable
{
    ...
    /// <summary>Applies reload-safe pipeline settings to this generation.</summary>
    /// <param name="state">Stable type identity and neutral property bytes.</param>
    public void Configure(SerializedRenderExtensionState state)
    {
        OnConfigure(state);
    }

    /// <summary>Builds all passes for one request.</summary>
    /// <param name="context">Frame-scoped pipeline context.</param>
    public abstract void Build(RenderPipelineContext context);

    /// <summary>Releases generation-scoped pipeline state.</summary>
    public void Dispose()
    {
        if (m_disposed)
            return;
    ...
    /// <summary>Reads pipeline-owned settings from neutral state.</summary>
    /// <param name="state">Reload-safe extension state.</param>
...
/// <summary>Adds capability-aware passes without owning frame graph state.</summary>
public abstract class RenderPipelineFeature
{
    /// <summary>Applies reload-safe settings to this feature generation.</summary>
    /// <param name="configuration">Stable feature configuration.</param>
    ...
    /// <summary>Adds frame-scoped passes and dependencies.</summary>
    /// <param name="context">Frame-scoped feature context.</param>
    public abstract void AddRenderPasses(RenderFeatureContext context);

    /// <summary>Reads feature-owned settings from neutral state.</summary>
    /// <param name="state">Reload-safe extension state.</param>
    protected virtual void OnConfigure(SerializedRenderExtensionState state) { }
}
