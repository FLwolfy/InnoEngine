...
public sealed unsafe class BgfxImGuiRenderer : IPlatformImGuiRenderer, IRenderFrameGraphContributor
{
    ...
    /// <summary>Creates a frame-contributing ImGui renderer around a backend-neutral shader artifact.</summary>
    /// <param name="device">Active BGFX-backed neutral device.</param>
    /// <param name="pipelineDescriptor">Compiled ImGui shader stages and reflected interface.</param>
    /// <exception cref="ArgumentException">Thrown when the shader artifact does not use the required ImGui layout.</exception>
    /// <exception cref="NotSupportedException">Thrown when <paramref name="device"/> is not BGFX-backed.</exception>
    ...
    /// <summary>Gets the exact interleaved vertex layout required by the built-in ImGui shaders.</summary>
    public static RenderVertexLayout vertexLayout => S_VERTEX_LAYOUT;

    /// <summary>Gets the last recoverable shader replacement error while the last-good pipeline remains active.</summary>
    public string? lastShaderError { get; private set; }

    /// <inheritdoc />
    public bool supportsViewports => true;

    /// <summary>Queues a compiled shader replacement that commits atomically at a frame safety point.</summary>
    /// <param name="pipelineDescriptor">Complete candidate artifact.</param>
    ...
    /// <summary>Registers a persistent render texture as an opaque ImGui texture token.</summary>
    /// <param name="texture">Texture owned by the active device generation.</param>
    /// <returns>An opaque token accepted by <see cref="PlatformImGuiContext.DrawImage"/>.</returns>
    ...
    /// <summary>Unregisters an opaque texture token without taking ownership of the source texture.</summary>
    /// <param name="texture">Previously allocated token.</param>
    /// <returns><see langword="true"/> when the token was an external registration.</returns>
