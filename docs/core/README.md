...
| 项目 | 职责 |
| --- | --- |
| [Inno.Core.Execution](Inno.Core.Execution.md) | 可撤销 execution slot、严格 LIFO、取消与资源 lifetime |
...
| [Inno.Core.Settings](Inno.Core.Settings.md) | Project Settings、中立有效值缓存、owner context 与冻结贡献组合 |

Core 不拥有 Shell、静态 Manager、程序集加载或脚本编译；这些职责分别属于 Composition、Runtime、Extensibility 和 Scripting。`Inno.Core.Layers` 只提供可复用生命周期原语，不拥有 Host 或产品资源。
