...
Game View 是开放 kind `inno.editor.viewport.game` 的通用 viewport host。活动 Plugin Provider 自行选择运行时数据、Pipeline、目标格式和交互行为；Panel 不查找内建 Camera，也不解释 Scene。Panel 只把当前 workspace 的有序 Scene 与 active Scene 作为 `RenderContentScope` 显式传入，具体单 Camera 或 Base/Overlay 合成由 Plugin 决定。
