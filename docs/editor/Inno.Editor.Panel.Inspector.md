...
EditorScripts 使用 `InnoEditor.Inspection`，可声明 InspectionDrawer、PropertyDrawer 并使用 draw context。Facade 由 `Inno.Editor.Inspection` 提供；本项目只补充引用 drop target，area/action 直接使用字符串。具体内建 Panel、Registry snapshot 和内部 metadata cache 不导出。
