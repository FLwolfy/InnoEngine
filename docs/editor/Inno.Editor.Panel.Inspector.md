...
Inspector 为所有可检查目标统一绘制无外部缝隙的 Target Header。Header 会抵消正文统一 content padding，背景与边框完整贴合 Inspector 顶部、左侧和右侧；内部仍使用自己的 header padding。Header 的大图标、名称、名称修改能力和第二行内容全部由当前 `InspectionDrawer<TTarget>` 提供；统一容器只负责布局、裁剪、边框和锁定。名称没有 setter 时直接显示为文字，不绘制输入框。第二行严格限制为一行，适合放置 active、tag、路径、标签或其他轻量目标信息。
...
Layer picker 只从 canonical `Assets/Settings/GameLayers.ilayers` 读取已命名 slot。资产移出 `Settings` 后仍作为普通文件存在，但不会被项目 layer 系统识别；Editor 立即使用只包含 `Default` 的内存配置，picker 只能选择 `Default`，已有对象引用自定义 slot 时发布 `GAMEOBJECT-LAYER-UNDEFINED`。文件以合理内容移回 canonical path 后会恢复对应定义并清除错误。修改对象 Layer 通过 `SceneEdits.SetGameObjectLayer` 记录稳定的 `GameLayer` 索引并进入 Undo/Redo，同时刷新 Scene layer query index。

单击 canonical `GameLayers.ilayers` 时，Inspector 会优先解析 exact drawer，可以直接定义 1–31 slot 的名称。按 Enter 或编辑后失焦都会通过 `AssetManager.Save` 提交到 canonical path，并产生正常 source/meta/artifact 更新；位于其他目录的 `.ilayers` 会显示位置错误且不可作为项目配置编辑。
