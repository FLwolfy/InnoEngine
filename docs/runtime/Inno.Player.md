...
8. 创建 MiniAudio device 与 `AudioRuntimeLayer`，应用 `AudioProjectSettings`；设备失败时进入明确 muted 状态并写诊断。
9. 从只读 Catalog 加载 startup `SceneAsset`，实例化到该 Session 的 `SceneWorld`。
10. 在 Assets/Settings/Audio/Session execution scopes 中推进帧；关闭时逆序释放 Audio、Rendering、Session、Settings、Host、GPU、Window 和 Platform。
...
Player closure 不允许包含 Roslyn、Editor、Build、Scripting Compiler/Reload、Assets Pipeline、Plugins Authoring、shaderc、texturec、C# source、Shader source、裸 Assets 或裸 Plugins。平台原生运行库位于应用布局的 `native/`，只包含执行游戏实际需要的 SDL3、BGFX 与 MiniAudio runtime binary。
...
- Audio diagnostics 同样进入实例 `LogRouter`；无声卡不会使游戏崩溃，Runtime 保持时钟和完成事件并明确报告 muted 状态。
