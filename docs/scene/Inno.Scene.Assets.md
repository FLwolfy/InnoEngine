...
该项目拥有 `SceneAsset`、`PrefabAsset`、对应 Importer，以及 Scene/Prefab graph serialization、override processing 和脚本对象 reload migration。Serialization namespace 统一为 `Inno.Scene.Assets`；它不是独立程序集。
...
AssetPipeline.Save("Scenes/Level.iscene", asset);
```
