...
Game Layer 与 Tag 定义不属于本项目。它们是普通的强类型 `ProjectSettingDefinition`，由 [Inno.Core.Settings](../core/Inno.Core.Settings.md) 以稳定 `ProjectSettingId` 写入项目根 `ProjectSettings.inno`；[Inno.Editor.Settings](../editor/Inno.Editor.Settings.md) 只提供对应 Drawer，不经过 AssetManager 或 Source Database，也没有 Scene 分类专用持久化通道。
