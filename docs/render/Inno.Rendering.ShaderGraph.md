...
- `ShaderGraphCompiler` 验证 Missing Node、连接、类型转换、阶段合法性和输出数量，使用 Core Collections DependencyGraph 生成确定性节点拓扑，然后生成普通 `ShaderIRModule`。
...
手动 `ShaderNodeRegistry.Replace` 要求候选实例与活动 generation 独立，拒绝借用当前 definition。候选发布后的旧快照通过 Core RetirementBarrier 退休；Pending/timeout 时保留旧 owner 并阻止后续读取/发布，不能先丢掉引用再报告失败。发现模式只由 TypeCatalog transaction 更新。Dispose 复用同一 lifetime，不吞扩展退出异常。

`ShaderGraphCompileResult.diagnostics`、`ShaderGraphEmission.properties/statements/semantics` 为复制后的只读集合。GraphDocument 是创作模型；编译上下文只在本次编译期间借用，不能把这个 mutable 模型描述成长期 immutable result。
