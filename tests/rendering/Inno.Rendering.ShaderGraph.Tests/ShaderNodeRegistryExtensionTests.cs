...
public sealed class ShaderNodeRegistryExtensionTests : IDisposable
{
    ...
    public ShaderNodeRegistryExtensionTests()
    {
        ...
        DisposableShaderNodeDefinition.pendingRetirements = 0;
    ...
    [Fact]
    public void ManualReplacementAndShutdownDrainPendingDefinitionsWithoutLosingOwnership()
    {
        var registry = new ShaderNodeRegistry();
        registry.Replace([new DisposableShaderNodeDefinition()]);
        DisposableShaderNodeDefinition.pendingRetirements = 2;
        registry.Replace([new DisposableShaderNodeDefinition()]);
        Assert.Equal(3, DisposableShaderNodeDefinition.disposeCount);
        DisposableShaderNodeDefinition.pendingRetirements = 1;
        registry.Dispose();
        registry.Dispose();
        Assert.Equal(5, DisposableShaderNodeDefinition.disposeCount);
    }
...
public sealed class DisposableShaderNodeDefinition : ShaderNodeDefinition, IDisposable
{
    ...
    {
        disposeCount++;
        if (pendingRetirements-- > 0) throw new RetirementPendingException("Shader node work is draining.");
    }
}
