...
public sealed class PluginSourceServiceTests : IDisposable
{
    ...
    [Fact]
    public void PublishedPluginSnapshotsDoNotExposeMutableCatalogOrManifestState()
    {
        WritePlugin("immutable.iplugin", Manifest("tests.immutable"), TextContent("sample"));
        ...
        candidate.manifest.pluginId = "mutated";
        candidate.manifest.contentRoots[0] = "elsewhere";
        Assert.Equal("tests.immutable", candidate.manifest.pluginId);
        Assert.Equal("Assets", Assert.Single(candidate.manifest.contentRoots));
        Assert.Throws<NotSupportedException>(() => ((IList<PluginCandidate>)scan.candidates).Clear());
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void CandidateObserverPendingKeepsTheCandidateAndStopsLaterObservers(bool earlierFailure)
    {
        InitializeProjectAssets();
        var service = new PluginSourceService(m_serialization, m_plugins, m_library);
        StartEnvironment(service.Scan());
        WritePlugin("pending-observer.iplugin", Manifest("tests.pending-observer"), new Dictionary<string, byte[]>
        {
            ["Assets/Plugin.cs"] = "public sealed class PendingObserverEntry { }"u8.ToArray(),
            ["Assets/Plugin.cs.imeta"] = CreateTextSourceMeta()
        });
        var ordinary = new InvalidOperationException("earlier observer failed");
        var pending = new AggregateException(new RetirementPendingException("observer still owns work"));
        bool laterCalled = false;
        Action first = () => throw ordinary;
        Action blocked = () => throw pending;
        Action later = () => laterCalled = true;
        if (earlierFailure) m_environment!.ActivationCandidateChanged += first;
        m_environment!.ActivationCandidateChanged += blocked;
        m_environment.ActivationCandidateChanged += later;
        try
        {
            AggregateException failure = Assert.Throws<AggregateException>(() => m_environment.Refresh());
            if (earlierFailure)
            {
                Assert.Contains(ordinary, failure.InnerExceptions);
                Assert.Contains(pending, failure.InnerExceptions);
            }
            else Assert.Same(pending, failure);
            Assert.False(laterCalled);
            Assert.True(m_environment.hasPendingActivation);
            Assert.NotNull(m_environment.compilationAssets);
            Assert.Empty(m_environment.activePlugins);
            Assert.Single(m_assets!.sourceMounts);
        }
        finally
        {
            // The simulated observer has quiesced; explicitly retire the retained candidate.
            m_environment.ActivationCandidateChanged -= first;
            m_environment.ActivationCandidateChanged -= blocked;
            m_environment.ActivationCandidateChanged -= later;
            m_environment.RollbackPending();
        }
    ...
    [Fact]
    public void RollbackObserverPendingKeepsTheOwnerUntilAllRollbackStagesComplete()
    {
        InitializeProjectAssets();
        var service = new PluginSourceService(m_serialization, m_plugins, m_library);
        StartEnvironment(service.Scan());
        WritePlugin("pending-rollback.iplugin", Manifest("tests.pending-rollback"), new Dictionary<string, byte[]>
        {
            ["Assets/Plugin.cs"] = "public sealed class PendingRollbackEntry { }"u8.ToArray(),
            ["Assets/Plugin.cs.imeta"] = CreateTextSourceMeta()
        });
        Assert.True(m_environment!.Refresh());
        m_environment.ActivatePending();
        AssetSourceMountTransaction candidate = m_environment.compilationAssets!;
        var failure = new InvalidOperationException("observer context", new RetirementPendingException("rollback work"));
        bool laterCalled = false;
        Action blocked = () => throw failure;
        Action later = () => laterCalled = true;
        m_environment.Changed += blocked;
        m_environment.Changed += later;
        try
        {
            Assert.Same(failure, Assert.Throws<InvalidOperationException>(m_environment.RollbackPending));
            Assert.True(m_environment.hasPendingActivation);
            Assert.Same(candidate, m_environment.compilationAssets);
            Assert.False(laterCalled);
        }
        finally
        {
    ...
    [Fact]
    public void SourceMountCandidate_TransfersLiveIdentitiesOnlyDuringActivation()
    {
        const string c_fileName = "identity-handoff.iplugin";
        WritePlugin(c_fileName, Manifest("tests.identity-handoff"), new Dictionary<string, byte[]>
        {
            ["Assets/Plugin.cs"] = "public sealed class IdentityHandoffV1 { }"u8.ToArray(),
            ["Assets/Plugin.cs.imeta"] = CreateTextSourceMeta()
        });
        InitializeProjectAssets();
        var service = new PluginSourceService(m_serialization, m_plugins, m_library);
        StartEnvironment(service.Scan());
        Assert.True(m_assets!.TryGetFileSystemEntry(
            AssetPath.Project(string.Empty),
            out AssetFileEntry activeEntry));
        Assert.NotNull(activeEntry.identity.runtimeIdentity);

        WritePlugin(c_fileName, Manifest("tests.identity-handoff"), new Dictionary<string, byte[]>
        {
            ["Assets/Plugin.cs"] = "public sealed class IdentityHandoffV2 { }"u8.ToArray(),
            ["Assets/Plugin.cs.imeta"] = CreateTextSourceMeta()
        });

        Assert.True(m_environment!.Refresh());
        AssetSourceMountTransaction candidate = Assert.IsType<AssetSourceMountTransaction>(
            m_environment.compilationAssets);
        AssetFileEntry candidateEntry = Assert.Single(
            candidate.GetFileSystemEntries(),
            static entry => entry.assetPath == AssetPath.Project(string.Empty));
        Assert.Equal(activeEntry.identity.persistentId, candidateEntry.identity.persistentId);
        Assert.NotNull(activeEntry.identity.runtimeIdentity);
        Assert.Null(candidateEntry.identity.runtimeIdentity);

        m_environment.ActivatePending();

        Assert.Null(activeEntry.identity.runtimeIdentity);
        Assert.True(m_assets.TryGetFileSystemEntry(
            AssetPath.Project(string.Empty),
            out AssetFileEntry activatedEntry));
        Assert.Equal(activeEntry.identity.persistentId, activatedEntry.identity.persistentId);
        Assert.NotNull(activatedEntry.identity.runtimeIdentity);

        m_environment.RollbackPending();

        Assert.NotNull(activeEntry.identity.runtimeIdentity);
        Assert.Null(activatedEntry.identity.runtimeIdentity);
        Assert.True(m_assets.TryGetFileSystemEntry(
            AssetPath.Project(string.Empty),
            out AssetFileEntry restoredEntry));
        Assert.Same(activeEntry, restoredEntry);
    }
