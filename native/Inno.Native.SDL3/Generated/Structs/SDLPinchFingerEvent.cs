// ------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
// ------------------------------------------------------------------------------

using System;
using System.Diagnostics;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;
using BGCS.Runtime;

namespace Inno.Native.Sdl3
{
	/// <summary>
	/// Pinch event structure (event.pinch.*)<br/>
	/// </summary>
	[NativeName(NativeNameType.StructOrClass, "SDL_PinchFingerEvent")]
	[StructLayout(LayoutKind.Sequential)]
	public partial struct SDLPinchFingerEvent
	{
		/// <summary>
		/// ::SDL_EVENT_PINCH_BEGIN or ::SDL_EVENT_PINCH_UPDATE or ::SDL_EVENT_PINCH_END <br/>
		/// </summary>
		[NativeName(NativeNameType.Field, "type")]
		[NativeName(NativeNameType.Type, "SDL_EventType")]
		public SDLEventType Type;

		[NativeName(NativeNameType.Field, "reserved")]
		[NativeName(NativeNameType.Type, "Uint32")]
		public uint Reserved;
		/// <summary>
		/// In nanoseconds, populated using SDL_GetTicksNS() <br/>
		/// </summary>
		[NativeName(NativeNameType.Field, "timestamp")]
		[NativeName(NativeNameType.Type, "Uint64")]
		public ulong Timestamp;

		/// <summary>
		/// The scale change since the last SDL_EVENT_PINCH_UPDATE. Scale <br/>
		/// <<br/>
		/// 1 is "zoom out". Scale > 1 is "zoom in". <br/>
		/// </summary>
		[NativeName(NativeNameType.Field, "scale")]
		[NativeName(NativeNameType.Type, "float")]
		public float Scale;

		/// <summary>
		/// The window underneath the finger, if any <br/>
		/// </summary>
		[NativeName(NativeNameType.Field, "windowID")]
		[NativeName(NativeNameType.Type, "SDL_WindowID")]
		public uint WindowID;


		public unsafe SDLPinchFingerEvent(SDLEventType type = default, uint reserved = default, ulong timestamp = default, float scale = default, uint windowID = default)
		{
			Type = type;
			Reserved = reserved;
			Timestamp = timestamp;
			Scale = scale;
			WindowID = windowID;
		}


	}

}
